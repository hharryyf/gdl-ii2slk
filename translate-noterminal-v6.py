...
def print_evolutions(maxd:int = 3, recall:int = 1):
    ...
    for r in legal.keys():
        ...
        for a in legal[r]:
            if len(no_play_r) == 0:
                no_play_r = f'legal_{r}_{a} = false'
            else:
                no_play_r += f' and legal_{r}_{a} = false'
        if len(no_play) == 0:
            no_play = f'({no_play_r})'
        else:
            no_play += f' or ({no_play_r})' 
    print('    Evolution:')
    print(f'        -- print the counters')
    print('        ok = 0 if (ok = 0);')
    print(f'        (counter = counter + 1) if !((terminal = true and counter = 0) or counter = {maxd} or (counter = {maxd-2} and act_step = false) or (counter = 0 and act_step = true and ({no_play})));')
    print(f'        (counter = 0) if ((terminal = true and counter = 0) or counter = {maxd} or (counter = {maxd-2} and act_step = false) or (counter = 0 and act_step = true and ({no_play})));')

...
def print_evolutions(maxd:int = 3, recall:int = 1):
    ...
    print(f'        (counter = counter + 1) if !(init <> 0 or (terminal = true and counter = 0) or counter = {maxd} or (counter = {maxd-2} and act_step = false));')
    print(f'        (counter = 0) if (init <> 0 or (terminal = true and counter = 0) or counter = {maxd} or (counter = {maxd-2} and act_step = false));')
    print(f'        act_step = false if ((init > 1) or (counter < {maxd-2} and init = 0 and act_step = false) or (counter = {maxd} and init = 0 and act_step = true));')
    print(f'        act_step = true if !((init > 1) or (counter < {maxd-2} and init = 0 and act_step = false) or (counter = {maxd} and init = 0 and act_step = true));')    
    print()
    ...
    for r in sorted(see.keys()):
        for observations in sorted(see[r]):
            for i in range(recall+1):
                if i == 0:
                    print(f'        seen_{r}_{observations}_{i+1} = sees_{r}_{observations} if ((init = 0 and act_step = true and counter = {maxd}));')
                    print(f'        seen_{r}_{observations}_{i+1} = seen_{r}_{observations}_{i+1} if !((init = 0 and act_step = true and counter = {maxd}));')
                else:
                    print(f'        seen_{r}_{observations}_{i+1} = seen_{r}_{observations}_{i} if ((init = 0 and act_step = true and counter = {maxd}));')
                    print(f'        seen_{r}_{observations}_{i+1} = seen_{r}_{observations}_{i+1} if !((init = 0 and act_step = true and counter = {maxd}));')
